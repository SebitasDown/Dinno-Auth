        ...
        return userRepository.findByEmail(registerCommand.email())
                .flatMap(existingUser ->{
                    if (existingUser.isDeleted()){
                        return Mono.error(new AccountDeleteException("No puedes usar este email: la cuenta asociada está en proceso de eliminación"));
                    }
                    return Mono.error(new EmailAlreadyExistException("El correo electronico " + registerCommand.email() + " ya se encuentra registrada"));
                })
